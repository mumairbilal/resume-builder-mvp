// In production (Vercel etc.) set VITE_API_URL to your deployed backend's
// URL, e.g. https://your-backend.onrender.com — falls back to localhost
// for local development so nothing breaks if it's not set.
...
// ---- ATS Score ----
export const scoreSavedResumeATS = (resumeId, jobDescription) =>
  request(`/ats/score/${resumeId}`, { method: "POST", body: { job_description: jobDescription } });

export async function scoreUploadedResumeATS(file, jobDescription) {
  const headers = {};
  const token = getToken();
  if (token) headers["Authorization"] = `Bearer ${token}`;
  const formData = new FormData();
  ...
  const res = await fetch(`${BASE_URL}/ats/score-upload`, { method: "POST", headers, body: formData });
  let data;
  try {
    data = await res.json();
  } catch {
