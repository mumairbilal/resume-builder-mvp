...
_stats_cache = TTLValue(ttl_seconds=600)  # 10 minutes
...
def get_stats(db: Session = Depends(get_db), _=Depends(get_current_admin)):
    # Cached for 30s: these are several aggregate queries scanning the
    # whole users/resumes tables — expensive to run on every dashboard
    # refresh, and a few seconds of staleness on an admin overview is fine.
    hit = _stats_cache.get()
    if hit is not None:
        return hit
